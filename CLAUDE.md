...
`db_data`, `app_data`, and `caddy_data` are all defined with `driver: local` and `driver_opts: {type: none, o: bind, device: ${PWD}/data/db}` (respectively `.../data/app`, `.../data/caddy`) — still named volumes as far as Docker's own bookkeeping is concerned (`docker volume ls`/`inspect` show them, `worker` and `app` still share `app_data` purely by volume name), but the backing storage is an explicit host directory next to `docker-compose.yml` instead of Docker's internal `/var/lib/docker/volumes/...`. That directory must already exist before `docker compose up` — the `local` driver's bind option does not create it — which is why `make up` runs `mkdir -p data/db data/app data/caddy`. Editing files on the host under `data/app` **does** now reach the running container. Ownership self-heals on every start regardless: `entrypoint.sh` runs `chown -R www-data:www-data /apps` as root before handing off to Apache, and the official `mariadb` image's own entrypoint chowns its datadir the same way. Caddy runs as its own non-root user inside the container and manages `/data`'s ownership itself — no equivalent chown step needed here.

`backup_repo` uses the same `driver: local` bind pattern but a deliberately different, sibling path: `${PWD}/backups`, not under `data/` at all (see "Backups: restic" below for why) — and unlike the three above, it's not in `make up`'s `mkdir -p` list.
...
- **`backup_entrypoint`** runs once at container start: `apk add --no-cache mariadb-client` (the base image is Alpine but doesn't ship a MySQL/MariaDB client — confirmed by inspecting the image directly), ensures `/backup/repo` exists and `/backup/restic-password` is populated (generating one with `head -c 32 /dev/urandom | base64` if missing — `base64` confirmed present in the image), writes a crontab entry (`0 3 * * * sh /scripts/run.sh`), and execs `crond -f -d 8` as the container's foreground process. `restic/restic` already bundles `crond` (busybox), so no separate scheduler container/image is needed.
- **No `RESTIC_PASSWORD` in `.env` at all** — `RESTIC_REPOSITORY=/backup/repo` and `RESTIC_PASSWORD_FILE=/backup/restic-password` both point inside the `backup_repo` volume itself, so the password lives right next to the repository it encrypts rather than in `.env`. This was a deliberate choice after discussion: restic has no unencrypted mode, but the user didn't want to manage a separate secret, so self-generating the password onto the same off-host (NFS) storage means it can never be "lost" independently of the backups — the tradeoff being that anyone with access to the backup storage has both the data and the key, which is acceptable for private off-host storage the way it wouldn't be for, say, a public S3 bucket.
- **`backup_run`** (the actual daily job): `mariadb-dump`s the `db` service over the network into a scratch dir (no Docker socket access needed — this is why `backup` isn't given DB credentials beyond what `app`/`worker` already receive the same way), then `restic backup`s that dump plus the read-only-mounted `app_data` and `caddy_data` volumes, then `restic forget --keep-daily 7 --keep-weekly 4 --keep-monthly 6 --prune`. The database is deliberately backed up via a logical dump rather than by including raw `db_data` files in the restic backup — MariaDB is live while this runs, and a raw file copy of an in-use InnoDB datadir isn't guaranteed consistent, whereas `mariadb-dump` produces a consistent logical snapshot through the MySQL protocol. `data/app` and `data/caddy` don't have that concern, so they're backed up as plain files.
- **`--host xivarricrm` is pinned explicitly on both `restic backup` and `restic forget`.** Without it, restic defaults to the container's own hostname (its container ID), and since `restart: unless-stopped` containers get a *new* hostname every time they're recreated (image update, config change), restic's retention policy — which groups/prunes per hostname by default — would silently start a brand-new retention group on every recreation instead of extending the existing one. Caught this empirically: added `caddy_data` to the backup, the container got recreated, and the next `restic snapshots` showed the new snapshot under a different `Host` value than the previous one, with `restic forget` then only "keep 1" *per host* rather than enforcing 7/4/6 across the whole history. Fixed by pinning `--host` to a stable value instead of relying on the ambient hostname.
...
- **Why `backups/` isn't auto-`mkdir`'d**: it's meant to be an NFS mount (or other off-host storage) the operator sets up *before* starting the stack, unlike `data/db`/`data/app`/`data/caddy` which are meant to be plain local directories. If it doesn't exist, `docker compose up` fails loudly (same failure mode already established for the other `driver_opts` bind volumes) rather than silently writing "backups" to a local folder that provides no protection against a disk failure. It's also deliberately a sibling of `data/`, not nested under it — makes it visually obvious at a glance that this one is off-host storage, not part of the live app state.
